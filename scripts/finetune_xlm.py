...
def evaluate_kfold(args, data_silo, processor):
    ...
    for num_fold, silo in enumerate(silos):
        ...
        evaluator_test = MultitaskEvaluator(
            data_loader=silo.get_data_loader(f"test"), tasks=silo.processor.tasks, device=args.device
        )
        ...
        for res in result[1:]:
            ...
            dict_preds_labels[res["task_name"]]["preds"].extend(preds)
            dict_preds_labels[res["task_name"]]["labels"].extend(labels)

            with open(f'out/preds_{args.run_name}_{num_fold}.tsv', 'a', encoding='utf8') as preds_file:
                for pred, label in zip(preds, labels):
                    preds_file.write(res['task_name'] + '\t' +  str(pred) + '\t' + str(label) + '\n')

    # Write out final results
    results_path = f'out/results_{args.experiment_name}.tsv'
    ...
    with open(results_path, 'w', encoding='utf8') as outfile:
        ...
        for task_name, task in dict_preds_labels.items():
            ...
            outfile.write(args.run_name + '\t' + task_name + '\t' +  str(metrics['max_err']) + '\t' + str(metrics['mae']) + '\t' + str(metrics['mse']) + '\t' + str(metrics['rmse']) + '\t' + str(metrics['r2']) + '\t' + str(metrics['err_std']) + '\n')
...
def finetune_sentence_level(args):
    ...
    if args.folds > 1:
        ...
        if not args.do_eval_only:
            ...
            results_path = f'./results/eye-movement-prediction/results_{args.experiment_name}.tsv'
            preds_path = f'./results/eye-movement-prediction/preds_{args.run_name}.tsv'

            # write regression metrics to outfile
            if not os.path.exists(results_path):
                with open(results_path, 'w', encoding='utf8') as outfile:
                    ...
                    for res in result[1:]:
                        ...
                        outfile.write(
                            args.run_name + '\t' + res['task_name'] + '\t' + str(metrics['max_err']) + '\t' + str(
                ...
                with open(results_path, 'a', encoding='utf8') as outfile:
                    for res in result[1:]:
...
def main():
    ...
    parser.add_argument(
        "--run_name", default='train_xlm_on_geco', type=str, required=True, help="The name of the experiment to be saved in MLflow"
    )
    parser.add_argument(
        "--data_dir",
        default='./data/geco/train_test',
        type=str,
        required=True,
    ...
    parser.add_argument(
        "--save_dir",
        default="./models/xlm_trained_on_geco",
        type=str,
    ...
    parser.add_argument(
        "--experiment_name",
        default="train_xlm_on_geco",
        type=str,
        help="The name of the experiment to be saved in MLflow",
    )
    parser.add_argument("--folds", type=int, default=5, help="Number of Cross-Validation Folds")
    parser.add_argument("--text_column", type=str, default="text",
                        help="Name of the text column to be used for training. Default: %(default)s")
    parser.add_argument(
        "--label_columns",
        nargs="+",
        required=True,
